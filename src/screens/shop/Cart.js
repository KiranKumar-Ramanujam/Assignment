    ...
    <SafeAreaView style={styles.sectionContainer}>
      <View style={styles.header}>
        <Text style={styles.title}>
          Total: <Text style={styles.total}>RM{totalAmount.toFixed(2)}</Text>
        </Text>
        <Button
          onPress={() => dispatch(addToOrders(cartProducts, totalAmount))}
          title="Order Now"
          color="blue"
      ...
      <FlatList
        ...
        renderItem={prod => {
          return (
            <CartItem
              title={prod.item.productTitle}
              keyExtractor={item => item.productId}
              sum={prod.item.sum}
              onDelete={() => dispatch(deleteFromCart(prod.item.productId))}
