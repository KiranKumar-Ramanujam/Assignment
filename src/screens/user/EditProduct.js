...
const EditProduct = props => {
  ...
  const [title, setTitle] = useState(
    productId !== undefined ? editedProduct.title : '',
  );
  const [imageUrl, setImageUrl] = useState(
    productId !== undefined ? editedProduct.imageUrl : '',
  );
  const [price, setPrice] = useState('');
  const [description, setDescription] = useState(
    productId !== undefined ? editedProduct.description : '',
  );
    ...
    <ScrollView>
      <View style={styles.form}>
        <View style={styles.formControl}>
          ...
          <TextInput
            onEndEditing={() => console.log('done editing :D')}
            returnKeyType="next"
            style={styles.input}
            value={title}
