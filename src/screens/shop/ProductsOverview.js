...
const ProductsOverview = props => {
  ...
  const renderItemHandler=(data)=>{
      return <ProductItem key={data.item.id} 
              ...
              >
                    <Button color="green" title="View Product" onPress={()=>props.navigation.navigate("ProductDetails",{
                            productId:data.item.id, ProductTitle:data.item.title,
                          })}/>
                    <Button color="black" title="Add to Cart" onPress={()=>dispatch(addToCart(data.item))}/>
              </ProductItem>
  }
